...
warnings:
  - "发布时封面图必须使用公网可访问的绝对URL（如 https://example.com/cover.jpg），禁止使用本地相对路径（如 ./assets/cover.jpg），否则远程 MCP 服务无法下载图片导致发布失败。"
---
...
- 支持设置标题、作者、摘要
- ⚠️ **重要**：封面图 `cover` 必须使用公网绝对 URL（如 `https://picsum.photos/900/383`），禁止使用本地相对路径（`./assets/cover.jpg`）
